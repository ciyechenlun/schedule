...
public class ConcatPinyin {
	...
	public static String changeToPinyin(String userName) {
		if (userName.length() == 0) {
			...
			for (int i = 0; i < userName.length(); i++) {
				...
				boolean zimu = (hanzi >= 'a' && hanzi <= 'z')
										|| (hanzi >= 'A' && hanzi <= 'Z');
				if ((hanzi >= '0' && hanzi <= '9')
						|| zimu) {
					result += hanzi;
				} else {
					result = pinyinHandle(result, hanzi);
				}
			}
			return result;
		}
	...
	private static String pinyinHandle(String result, char hanzi) {
		try {
		...
		if (userName.length() == 0) {
			...
			for (int i = 0; i < userName.length(); i++) {
				...
				if ((hanzi >= '0' && hanzi <= '9')
					...
					result = pinyinHandleFirst(result, hanzi);
				}
			}
			return result;
		}
	...
	private static String pinyinHandleFirst(String result, char hanzi) {
		try {
			String[] pinyin = PinyinHelper.toHanyuPinyinStringArray(hanzi);// 转化后的结果,音调可能不同
			if (pinyin.length == 0) {
				result += "";// 如果没有转化成功,则用空格代替
