...
public class UserAccessLogFilter extends GenericFilterBean {
	...
	protected void replacePlaceholders(HttpServletRequest request,
		...
		while (matcher.find()) {
			judge(request, response, requestURI, userCode, remoteAddr, agent,
					requestMethod, requestTime, cupTime, output, matcher);
		}
	...
	private void judge(HttpServletRequest request,
			HttpServletResponse response, String requestURI, String userCode,
			String remoteAddr, String agent, String requestMethod,
			long requestTime, long cupTime, StringBuffer output, Matcher matcher) {
